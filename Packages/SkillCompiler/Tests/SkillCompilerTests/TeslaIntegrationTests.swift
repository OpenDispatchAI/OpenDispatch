...
struct TeslaIntegrationTests {
    ...
    static func compiledIndex() throws -> CompiledIndex {
        let manifest = try YAMLSkillParser.parse(teslaYAML)
        let compiler = SkillCompiler(languages: ["en"])
        return try compiler.compile(manifests: [manifest])
    }
    ...
    func parseTeslaYAML() throws {
        ...
        #expect(manifest.bridgeShortcut == "OpenDispatch - Tesla")
    }

    // MARK: - Lock / Unlock

    @Test("'unlock my car' → vehicle.unlock")
    func unlockMyCar() throws {
        let index = try Self.compiledIndex()
        let results = query("unlock my car", index: index)
        #expect(results.first?.actionID == "vehicle.unlock")
    }

    @Test("'lock the tesla' → vehicle.lock")
    func lockTheTesla() throws {
        let index = try Self.compiledIndex()
    ...
    func highConfidenceExact() throws {
        let index = try Self.compiledIndex()
